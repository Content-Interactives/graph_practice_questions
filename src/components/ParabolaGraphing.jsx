...
const ParabolaGraphing = forwardRef((props, ref) => {
	const [hoverPreview, setHoverPreview] = useState(null);
	const [pointsHistory, setPointsHistory] = useState([]);
	const [historyIndex, setHistoryIndex] = useState(0);
	const [parabolaProgress, setParabolaProgress] = useState(0);
	const [animatingParabolaIndex, setAnimatingParabolaIndex] = useState(null);
	...
	useImperativeHandle(ref, () => ({
		getStudentDrawingData() {
			const parabolas = allParabolasData.slice(-MAX_PARABOLAS).map((p) => ({
				vertex: { x: p.vertex.x, y: p.vertex.y },
				secondPoint: { x: p.secondPoint.x, y: p.secondPoint.y },
				h: p.h,
				k: p.k,
				a: p.a,
			}));
			return { parabolas, domain: { min: MIN, max: MAX } };
		},
		reset() {
			setPointsHistory([]);
			setHistoryIndex(0);
			...
			<svg width={WIDTH} height={HEIGHT} style={{ display: 'block', pointerEvents: 'none' }}>
				...
				{visibleParabolasFull.map(({ h, k, a, vertex, secondPoint }, idx) => {
					...
					return (
						<g key={idx} clipPath="url(#plot-clip-parabola)">
							<path d={data.pathDLeft} fill="none" stroke="#1967d2" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" />
							<path d={data.pathDRight} fill="none" stroke="#1967d2" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" />
							<polygon points={arrowPoints(data.leftArrow.sx, data.leftArrow.sy, data.leftArrow.dx, data.leftArrow.dy, PARABOLA_ARROW_SIZE)} fill="#1967d2" />
							<polygon points={arrowPoints(data.rightArrow.sx, data.rightArrow.sy, data.rightArrow.dx, data.rightArrow.dy, PARABOLA_ARROW_SIZE)} fill="#1967d2" />
							{vertex && (
								<circle cx={valueToX(vertex.x)} cy={valueToY(vertex.y)} r={POINT_RADIUS} fill="#1967d2" stroke="#fff" strokeWidth={2} />
							)}
							{secondPoint && (
								<circle cx={valueToX(secondPoint.x)} cy={valueToY(secondPoint.y)} r={POINT_RADIUS} fill="#d32f2f" stroke="#fff" strokeWidth={2} />
							)}
						</g>
					);
				})}
				{ghostParabolaData && (
					<g clipPath="url(#plot-clip-parabola)">
						<path d={ghostParabolaData.pathD} fill="none" stroke="#1967d2" strokeWidth={2.5} strokeOpacity={0.5} strokeDasharray="8 6" strokeLinecap="round" strokeLinejoin="round" />
						<polygon points={arrowPoints(ghostParabolaData.leftArrow.sx, ghostParabolaData.leftArrow.sy, ghostParabolaData.leftArrow.dx, ghostParabolaData.leftArrow.dy, PARABOLA_ARROW_SIZE)} fill="#1967d2" fillOpacity={0.5} />
						<polygon points={arrowPoints(ghostParabolaData.rightArrow.sx, ghostParabolaData.rightArrow.sy, ghostParabolaData.rightArrow.dx, ghostParabolaData.rightArrow.dy, PARABOLA_ARROW_SIZE)} fill="#1967d2" fillOpacity={0.5} />
					</g>
				)}
				{animatingParabolaData && (() => {
					...
					return (
						<g clipPath="url(#plot-clip-parabola)">
							<path d={data.pathDLeft} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - parabolaProgress} fill="none" stroke="#1967d2" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" />
							<path d={data.pathDRight} pathLength={1} strokeDasharray={1} strokeDashoffset={1 - parabolaProgress} fill="none" stroke="#1967d2" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" />
							{parabolaProgress >= 1 && (
								<>
									<polygon points={arrowPoints(data.leftArrow.sx, data.leftArrow.sy, data.leftArrow.dx, data.leftArrow.dy, PARABOLA_ARROW_SIZE)} fill="#1967d2" />
									<polygon points={arrowPoints(data.rightArrow.sx, data.rightArrow.sy, data.rightArrow.dx, data.rightArrow.dy, PARABOLA_ARROW_SIZE)} fill="#1967d2" />
...
ParabolaGraphing.displayName = 'ParabolaGraphing';
